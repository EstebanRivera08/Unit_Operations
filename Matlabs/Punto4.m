...
sprintf('Qr: %.2f, kJ/h', Qr)
sprintf('Qc: %.2f, kJ/h', Qc)
...
%% PUNTO 4
clear EtX EtY
HdeltaW = (Fh(xW)-Qr*0.8/W) ;
Obj5 = @(x) ((HdeltaW - hF)/(xW-zF))-((Fh(x) - HdeltaW)/(x - xW));
xcorte2 = fsolve(Obj5, 0.25);
Objetivo = @(HD) (HD-hF)/(xD-zF) - (HdeltaW-hF)/(xW-zF) ;
HdeltaD = fsolve(Objetivo, -15) ;

%Se determinan las parejas de puntos para las nuevas operatorias
%Valores de x tomados para trazar líneas de operación: 
x_xD = linspace(xcorte2, xD-0.001, 20);
x_xW = linspace(xW+0.001, xcorte2, 20);
%Se calcula el y que completa el punto sobre la línea de operación para
%cada x (vectores x_xD y x_xW)
for i = 1 : 20
    Obj6_D = @(y) ((FH(y) - HdeltaD)/(y - xD))-((Fh(x_xD(i)) - HdeltaD)/(x_xD(i) - xD));
    y_xD(i) = fsolve(Obj6_D, 0.5);
    
    Obj6_W = @(y) ((FH(y) - HdeltaW)/(y - xW))-((Fh(x_xW(i)) - HdeltaW)/(x_xW(i) - xW));
    y_xW(i) = fsolve(Obj6_W, 0.1);
end
clc
%Con las parejas de puntos se crean las funciones para las líneas de
%operación
...
nexttile
plot(x1,HL,'b',y1,HV,'r',xD,HdeltaD,'ok',xW,HdeltaW,'ok')
...
Rpunto4 = (HdeltaD - FH(xD))/((FH(xD)-Fh(xD)));
Rel = Rpunto4/Rmin ;
...
xlim([0 Et])
xlabel('No. Etapa')
ylabel('Fracción molar de Etanol, x,y')
grid minor
...
%Calor condensador
Qc = (HdeltaD - Fh(xD))*D ; %kJ/s
%Calor Rehervidor
Qr = (-HdeltaW + Fh(xW))*W ; %kJ/s


%% Información

sprintf('Qr: %.2f, kW', Qr/3600)
sprintf('Qc: %.2f, kW', Qc/3600)
sprintf('Num etapas: %.2f', Et)
sprintf('W: %.2f, kmol/h', W/1000)
sprintf('D: %.2f, kmol/h', D/1000)
sprintf('L1: %.2f, kmol/h', L(2)/1000)
...
format long
sprintf('M vap: %f, kg/s', m_vap)
sprintf('M liq: %f, kg/s', m_liq)
