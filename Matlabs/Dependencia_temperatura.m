...
for i=1:4 %filas son temperaturas
...
for j=1:4
    ...
    plot(X_eq(:,j),Y_eq(:,j),'k',XA_opei(:,j),YA_opei(:,j),'b',X_min(:,j),YA_min(:,j),'r',...
    XA1i(j),YA1,'ob',XA1_eq(j),YA1,'or',XA2,YA2,'ok',...
...
for j=1:4 
    ...
    Eta = find(X_etapas(:,j))-1; %Etapas a graficar para cada caso
    
    plot(Eta,X_etapas(Eta+1,j),Eta,Y_etapas(Eta+1,j),Eta,ones(size(Eta))*XA1i(j),'--',Eta,ones(size(Eta))*YA1,'--') 
