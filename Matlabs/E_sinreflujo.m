...
% la linea R_Np - S
HC_Aeq = linspace(wRNp_M+0.015 ,HC_A(end),100) ;
...
plot([wRNp_A;wS_A; max(xinter)], [LinRNpS(wRNp_A);LinRNpS(wS_A); LinRNpS(max(xinter))],...
...
plot([wRNp_A, XEmax],[wRNp_M, YEmax],'color',[0.4660 0.6740 0.1880],'Linewidth',1.5)
plot([wF_A, wS_A],[wF_M, wS_M],'Color',[0.9290 0.6940 0.1250],'Linewidth',1.5)
plot(Mmax_A,Mmax_M,'ok','MarkerFaceColor','b')
plot(wF_A,wF_M,'ok','MarkerFaceColor',[0.9290 0.6940 0.1250])
plot(wS_A,wS_M,'ok','MarkerFaceColor',[0.6350 0.0780 0.1840])
plot(wRNp_A,wRNp_M,'ok','MarkerFaceColor','r')
xlim([0 xDeltaR+0.2])
