...
hold on
for i=1:20
    plot([x_xDmin(i) xD],[Fh(x_xDmin(i)) HdeltaD], 'm')
    %plot(y_xD(i),FH(y_xD(i)), 'or')

    plot([y_xWmin(i) xW],[FH(y_xWmin(i)) HdeltaW], 'g')
    %plot(x_xW(i),Fh(x_xW(i)), 'ob')
end
plot(zF, hF, 'O')
plot([xW xD],[HdeltaW HdeltaD], 'k')
hold off
...
grid minor
hold on
plot(x_xWmin, Op_Wmin(x_xWmin), 'g', x_xDmin, Op_Dmin(x_xDmin), 'm' )
hold off
...
HdeltaDmin = Rmin*((FH(xD)-Fh(xD))) + FH(xD);
