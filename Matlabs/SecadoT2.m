...
yyaxis right
plot(TG2_v,Ntog_v,'r',TG2_v,Htog_v,'b')
xlabel('T_{G2}, °C')
ylabel('N_{tOG}, H_{tOG}')
grid minor
legend('Z, m','N_{tOG}','H_{tOG}')
...
%%
figure('Color','White')
TW1_v = TW1*ones(size(TG2_v)) ;
TW2_v = TW2*ones(size(TG2_v)) ;
TG1_v = TG1*ones(size(TG2_v)) ;
...
plot(z(i,:),TS(i,:),'-o','Color',j)
plot(z(i,:),TG(i,:),'-o','Color',j)
end
xlabel('Z,m')
ylabel('T, °C')
grid minor
