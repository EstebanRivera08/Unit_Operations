...
%Calidad (V/F) y entalpía de las corrientes de entrada
Kf = @(X,T,n) (K(1,PF(n),T) - 1).*zF(n,1)./(X.*(K(1,PF(n),T) - 1) + 1)...
    + (K(2,PF(n),T) - 1).*zF(n,2)./(X.*(K(2,PF(n),T) - 1) + 1)...
    + (K(3,PF(n),T) - 1).*zF(n,3)./(X.*(K(3,PF(n),T) - 1) + 1)...
    + (K(4,PF(n),T) - 1).*zF(n,4)./(X.*(K(4,PF(n),T) - 1) + 1)...
    + (K(5,PF(n),T) - 1).*zF(n,5)./(X.*(K(5,PF(n),T) - 1) + 1);

X = zeros(16,1);

X(6) = fsolve(@(X) Kf(X,TF(6),6), 0.5);
X(9) = fsolve(@(X) Kf(X,TF(9),9), 0.5);

hFliq = zeros(16,5);
hFvap = zeros(16,5);
hFx = zeros(16,1);
hFy = zeros(16,1);
xF = zeros(16,5); %Composición líquido entradas
yF = zeros(16,5); %Composición vapor entradas

for j=1:16
    for i=1:1:5
        xF(j,i) = zF(j,i)/(1+(K(i,PF(j),TF(j)) - 1)*X(j));
        yF(j,i) = zF(j,i)*K(i,PF(j),TF(j))/(1+(K(i,PF(j),TF(j)) - 1)*X(j));
        hFliq(j,i) = xF(j,i)*Cpliq(i, TF(j)/2)*TF(j);
        hFvap(j,i) = yF(j,i)*(lambda(i,0) + integral(@(Tn) Cpvap(i,Tn),0,TF(j)));
    end
    hFx(j) = sum(hFliq(j,:));
    hFy(j) = sum(hFvap(j,:));
end

hF(6) = hFx(6) + X(6)*(hFy(6) - hFx(6));
hF(9) = hFx(9) + X(9)*(hFy(9) - hFx(9));

%Continuación de aproximación inicial de flujos de vapor
V(7) = V(6) - X(6)*F(6);
V(8) = V(7); V(9) = V(7);
...
for i = 1:5
nexttile
    plot(Etapas',[x(:,i).*L y(1:16,i).*V(1:16)],Etapas',...
        [xF(:,i).*F yF(:,i).*F -y(1:16,i).*W -x(:,i).*U])
    legend('Liquido', 'Vapor','F_L','F_V','W','U')
    xlabel('No. Etapas')
    Sustancia = ["Etano"; "Propano"; "n-Butano"; "n-Pentano"; "n-Hexano"] ;
    ylabel(sprintf(' m_{%s}, lbmol/h',Sustancia(i)))
    xlim([1 16])  
    grid minor
end
...
for i = 1:5
    ...
    for j = 1:6
    hold on
    plot(Etapas',ComposicionesLiq(:,i,j))
    end
        xlabel('No. Etapas')
    Sustancia = ["Etano"; "Propano"; "n-Butano"; "n-Pentano"; "n-Hexano"] ;
    ylabel(sprintf(' x_{%s} ',Sustancia(i)))
    xlim([1 16])  
    grid minor
    legend('i = 0','i = 1','i = 2','i = 3','i = 4','i = 5')
    hold off
...
for i = 1:5
    ...
    for j = 1:5
    hold on
    plot(Etapas',ComposicionesVap(1:16,i,j))
    end
    xlabel('No. Etapas')
    Sustancia = ["Etano"; "Propano"; "n-Butano"; "n-Pentano"; "n-Hexano"] ;
    ylabel(sprintf(' y_{%s} ',Sustancia(i)))
    xlim([1 16])  
    grid minor
...
function entalpia = hvap(n)
    ...
    entalpia=0;
    for i=1:1:5
        entalpia = entalpia + y(n,i)*(lambda(i,0) + integral(@(Tn) Cpvap(i,Tn),0,T(n)));
    end
end
